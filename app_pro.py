...
# --- CONEXÃO INTELIGENTE (SEM ERRO 404) ---
...
def gerar_resposta_inteligente(prompt, historico, midia=None):
    ...
    # PROMPT COM A MATEMÁTICA CORRIGIDA
    prompt_sistema = f"""
    Você é o AgroMind, o melhor consultor agronômico do Brasil.
    
    DIRETRIZES TÉCNICAS (MATEMÁTICA):
    1. FÓRMULA DE CALAGEM (Método Saturação por Bases):
       - A fórmula CORRETA é: NC (ton/ha) = (CTC x (V2 - V1)) / PRNT
       - Onde: CTC em cmolc/dm³, V2 e V1 em %, PRNT em %.
       - JAMAIS divida por 10 ou 100 se a CTC for em cmolc/dm³.
       - Se o usuário não informar o PRNT, pergunte qual é ou assuma 80% (avisando).
    ...
    try:
        ...
        return f"⚠️ O servidor está reiniciando. Tente de novo em 1 minuto. (Erro: {e})"
...
with aba1:
    ...
    if prompt_usuario or arquivo:
        ...
        # Envia histórico curto para economizar
...
with aba2:
    ...
    if noticias:
        for n in noticias:
            # Construção segura do HTML para evitar erro de aspas
            html_card = f"""
            <div class="news-card">
